...
class Copter:
    ...
    def __init__(self, default_stream_rate=1, sysid=1):
        self.wp_received = {}
        self.mav = None
    ...
    def wait_ready_to_arm(self, timeout=160, require_absolute=True, check_prearm_bit=True):
...
Also, ignores heartbeats not from our target system"""
    ...
    def get_distance_accurate(loc1, loc2):
        ...
        return Copter.gps_distance(loc1.lat, lon1, loc2.lat, lon2)
##########################
...
def main():
    ...
    sysid = int(os.getenv('SYSID', '1'))
    copter = Copter(sysid=sysid)

    big_print("Let's connect")
    # Assume that we are connecting to SITL
    instance = int(os.getenv('INSTANCE', '0'))
    #ip = socket.gethostbyname(socket.gethostname())
    ip = get_ip()
    #ip_interface = ipaddress.ip_interface(ip + "/24")
    connection_string = "tcp:" + str(ip) + ":" + str(5762 + 10 * instance)
    copter.connect(connection_string)
    ...
    print("MAVLINK_MSG_ID_GLOBAL_POSITION_INT rate : %f" %  copter.send_get_message_interval(ardupilotmega.MAVLINK_MSG_ID_GLOBAL_POSITION_INT))
    # We set the new rate
    copter.set_message_rate_hz(ardupilotmega.MAVLINK_MSG_ID_GLOBAL_POSITION_INT, 5)
    copter.set_parameter("SR0_POSITION", 20)
    copter.set_parameter("SR0_EXTRA1", 10)
    copter.set_parameter("SR1_POSITION", 20)
    copter.set_parameter("SR1_EXTRA1", 10)
    if sysid == 1:
        homepos = mavutil.location(-35.363261, 149.165230, 584, 353)
        wp_accuracy = copter.get_parameter("WPNAV_RADIUS", attempts=2)
        wp_accuracy = wp_accuracy * 0.01 + 1 # cm to m
        targetpos = mavutil.location(-35.363261, 149.165230, 584, 353)

        while True:
            targetpos.lat = homepos.lat + random.uniform(-0.001, 0.001)
            targetpos.lng = homepos.lng + random.uniform(-0.001, 0.001)
            targetpos.alt = homepos.alt + 20 + random.randint(-10, 10)
            ...
            copter.wait_location(targetpos, accuracy=wp_accuracy, timeout=240,
                                 target_altitude=targetpos.alt,
                                 height_accuracy=3, minimum_duration=2)
    do_follow = int(os.getenv('FOLL_ENABLE', '0'))
    if do_follow:
        position = instance % 10
        if position == 0:
            off_x = 8
            off_y = 0
        if position == 1:
        ...
        if position == 3:
            off_x = 8
            off_y = 0
        copter.set_parameter("FOLL_OFS_X", off_x)
        copter.set_parameter("FOLL_OFS_Y", off_y)

        big_print("Let's FOLLOW")
        copter.change_mode("FOLLOW")
        while True:
            foll_sysid = copter.get_parameter("FOLL_SYSID")
            if foll_sysid == 0:
                foll_sysid = int(os.getenv('FOLL_SYSID', '1'))
                copter.change_mode("GUIDED")
                copter.set_parameter("FOLL_SYSID", foll_sysid)
                copter.change_mode("FOLLOW")
            time.sleep(30)
